...
class Amt:
    ...
    def call(self, action, res, body="", sel=None):
        xml = ENV.format(
            addr=ADDR, wsman=WSMAN, xfer=XFER, anon=ANON,
            action=action, to=self.url, res=res,
            mid=uuid.uuid4(), selectors=self._selectors(sel), body=body,
        )
...
def epr(url, res, sel_name, sel_val):
    return (
        f"<a:Address>{url}</a:Address>"
...
def sync_clock(amt):
    ...
    x = amt.invoke(R_TIME, "GetLowAccuracyTimeSynch",
    ...
    now = int(time.time())
    amt.invoke(
        R_TIME, "SetHighAccuracyTimeSynch",
...
def commit(amt):
    print(">> CommitChanges")
    amt.invoke(R_SETUP, "CommitChanges",
...
def main():
    ...
    p.add_argument("--tls-only", action="store_true",
    ...
    body = (
        ...
        f"<r:ElementInCredentialContext>"
        f"{epr(amt.url, R_CERT, 'InstanceID', inst)}"
        f"</r:ElementInCredentialContext>"
        f"<r:ElementProvidingContext>"
        f"{epr(amt.url, R_TLSEP, 'ElementName', 'TLSProtocolEndpointInstances Collection')}"
        f"</r:ElementProvidingContext>"
    ...
    print(f"\nverify with:")
    print(f"  openssl s_client -connect {a.host}:16993 -tls1_2 "
